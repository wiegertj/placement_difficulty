...
tree_files = [file for file in all_files if file.endswith('_final_rb_support_pandit.raxml.support')]
results = []
for folder_name in tree_files:
    abs_path = os.path.abspath(os.path.join(folder_path, folder_name))
    dataset = folder_name.replace("_final_rb_support_pandit.raxml.support", "")
    print(abs_path)
...
df_res.to_csv(os.path.join(os.pardir, "data/rb_simulation_pandit.csv"))
