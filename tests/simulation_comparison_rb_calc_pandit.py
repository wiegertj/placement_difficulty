...
folder_path = '/hits/fast/cme/wiegerjs/PANDIT/wdir_rb'
import ete3
...
tree_files = [file for file in all_files if file.__contains__('bootstrap')]
results = []
for folder_name in tree_files:
    abs_path = os.path.abspath(os.path.join(folder_path, folder_name))
    dataset = folder_name.replace("RAxML_bootstrap.", "")
