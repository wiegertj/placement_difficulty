...
if os.path.exists(os.path.join(os.pardir, "data/loo_selection.csv")):
    ...
    difficulties_df = difficulties_df[difficulties_df["data_type"] == "AA" or difficulties_df["data_type"] == "DataType.AA"]
...
for i in range(len(difficulty_ranges) - 1):
    ...
    subset = difficulties_df[
        (difficulties_df['difficult'] >= lower_bound) & (difficulties_df['difficult'] < upper_bound)].sample(10)
    print("Subset size " + str(lower_bound) + " - " + str(upper_bound))
    print(subset.shape)
