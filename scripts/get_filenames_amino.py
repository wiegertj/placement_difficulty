...
difficulties_df = difficulties_df[difficulties_df["data_type"] == 'AA']
print(difficulties_df.shape)
...
for i in range(len(difficulty_ranges) - 1):
    ...
    if len(subset) < 4:
        ...
        selected_subset = subset.sample(4)
    print("Subset size " + str(lower_bound) + " - " + str(upper_bound))
    print(subset.shape)
...
for file in result["verbose_name"].unique():
    ...
    try:
        ...
        result = result[result['verbose_name'] != file]
        continue
if os.path.exists(os.path.join(os.pardir, "data/loo_selection_aa_test.csv")):
    result.to_csv(os.path.join(os.pardir, "data/loo_selection_aa_test.csv"), mode='a', header=False, index=False)
else:
    result.to_csv(os.path.join(os.pardir, "data/loo_selection_aa_test.csv"), header=True, index=False)
