...
def light_gbm_regressor(rfe=False, rfe_feature_n=20, shapley_calc=True):
    ...
    comparison_datasets = loo_selection["verbose_name"].str.replacer(".newick", "").values.tolist()
   ...
   # sample_dfs = random.sample(df["group"].unique().tolist(), int(len(df["group"].unique().tolist()) * 0.2))
    ...
    X_test_.to_csv(os.path.join(os.pardir, "data/prediction", "prediction_results" + "comparison_bs" + ".csv"))
