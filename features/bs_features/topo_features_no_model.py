...
for file in filenames:
    ...
    support_path_no_boot = os.path.join(grandir, "scripts/") + file.replace(".newick", "") + "_parsimony_1000_nomodel.raxml.startTree"
    support_path_boot = os.path.join(grandir, "data/raw/reference_tree/tmp_nomodel/") + file.replace(".newick", "") + "_pars_boot.txt"
    ...
    output_prefix_no_boot = file.split(".")[0] + "_parsimony_1000_nomodel"  # Using the filename as the prefix
    output_prefix_boot = file.split(".")[0] + "_parsimony_199_nomodel"  # Using the filename as the prefix
...
res_df.to_csv(os.path.join(grandir, "data/processed/features/bs_features/pars_top_features_no_model.csv"), index=False)