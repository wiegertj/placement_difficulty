    ...
    with open(support_file_path, "r") as support_file:
        ...
        for record in alignment:
            ...
            try:
                ...
                non_diff_match_counter_partb = (non_diff_match_counter_partb / len(alignment_b))  / (1 - sum(binary_results)) # how many non diff sites match diff in avg on part b?
            ...
            try:
                non_diff_match_counter_parta_thresh = (non_diff_match_counter_parta_thresh / len(alignment_a)) / (1 - sum(binary_results_threshold))
                non_diff_match_counter_partb_thresh = (non_diff_match_counter_partb_thresh / len(alignment_b)) / (1 - sum(binary_results_threshold)) # how many non diff sites match in avg on part b?
